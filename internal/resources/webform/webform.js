window.initGRPCForm = function(services, invokeURI, metadataURI, debug, headers) {
    ...
    function formMethodSelected(callback) {
        ...
        $.ajax(metadataURI + "?method=" + service + "." + method + (location.search ? "&" + location.search.substring(1) : ''))
            .done(function(data) {
    ...
    function invoke() {
        ...
        $.ajax(
            {
                type: "POST",
                url: invokeURI + "/" + service + "." + method + (location.search ? "?" + location.search.substring(1) : ''),
